...
1. **Per-lap segmentation** — detect lap boundaries from the live stream
...
2. **Setup data model** — define a `Setup` as
   `{id, name, car, track, fields: {…}, notes, created_at}` and store setups
   as JSON files in `setups/`. The 9 field sections are listed in step 3.
   A session references one setup.
3. **Setup editor (v1, all 9 categories)** — a single page in the dashboard
...
4. **Session index + simple session browser** — maintain a `sessions.json`
...
5. **Open past session** — clicking a session in the browser loads its
...
6. **Trace view (Advanced mode)** — timeline charts for speed / RPM /
...
7. **Simple / Advanced mode toggle** — top-bar switch in the dashboard.
   **Simple** = opinionated summary cards with friendly copy and a small
   ...
   (localStorage for v1; per-user account later if needed). [depends on 6]
8. **Per-widget expand + tooltip (layered on top of the toggle)** — every
...
9. **Lap comparison view** — overlay two laps' traces and show a delta-time
   chart underneath. The first "pro" feature that actually justifies the
   Advanced mode. [depends on 6]
10. **LLM: per-lap analysis mode** — extend `POST /api/insights` to accept a
    ...
    driving coaching. [depends on 2, 3]
11. **LLM: structured output (setup deltas)** — move the LLM response from
    ...
    [depends on 10]
12. **Setup comparison view** — pick two setups (current vs. previous /
    community), see the field diff, and overlay the laps run on each so the
    driver can judge whether the change actually helped. [depends on 3, 9]
13. **Sector analysis** — derive sector boundaries from position data
...
14. **Tire analytics** — per-tire temperature and slip statistics over a
    lap / stint. Detect cold/overheating tires, asymmetric setups
    (left vs. right imbalance), and tread-wear patterns. [depends on 1]
15. **Stint analytics** — fuel usage per lap, lap-time consistency (std
    dev / best 3-lap average), tire-deg trends across a stint. Surfaces in
    the insights panel for endurance races.
16. **Onboarding & first-run experience** — first-run wizard: pick your
    ...
    Sets the Simple/Advanced mode default per user. [depends on 7]
17. **Beginner-friendly explanations everywhere** — every metric in the UI
...
18. **Packaging & shareability** — verified clean `pip install -r
...
19. **Settings page** — `.env` is replaced in-app by a settings panel that
    writes back to `.env`: UDP host/port, LLM provider + key + model +
    base URL, log dir/stride/format, buffer size, default mode. Keeps the
    `.env` fallback for headless/server use. [depends on 18]
20. **In-app setup library** — a "My Setups" page listing all saved setups
